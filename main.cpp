...
void muestreoTemp(){
    ...
    lm35TempC = tempConvertida( lm35Promedio );
}


void CheckearSobrecalentamiento(){                             
    if(lm35TempC >= maxTempPermitida){      // si la temperatura exede de prendera un ventilador extrayendo el aire caliente.
        maxTempLED = ON;                                        
        normalTempLED = OFF;
        uartUsb.write("Alcanzó la máxima temperatura\r\n", 30); 
        while (lm35TempC > tempMedia){
            maxTempLED = ON;
            muestreoTemp();                    //Cuando se normalice el sistema, el ventilador se apaga
        }
        maxTempLED = OFF;                                   
        normalTempLED = ON;
...
void checkearRegistroTiempo(){                                            //Chequea el tiempo entre envíos de registros
    ...
    if(registroTiempoAcumulado == 86400){                              
        uartTask();                                           
        registroTiempoAcumulado = 0;
    }
}
